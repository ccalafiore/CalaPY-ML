...
class SequentialHeteroLayers(ModelMethods):
    ...
    def __init__(self, params_of_layers, device=None, dtype=None):
        ...
        if ModelMethods not in self.superclasses_initiated:
            ModelMethods.__init__(self=self)
            if ModelMethods not in self.superclasses_initiated:
                self.superclasses_initiated.append(ModelMethods)
...
class SequentialLSTMsSequentialFCLsParallelFCLs(ModelMethods):
